...
<?php
  if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["URL"]) && strpos($_GET["URL"], 'instagram.com') !== false) {
      ...
      echo '        var thumbnail = $(data).filter(\'meta[property="og:image"]\').attr("content");'."\n";
      echo '        var title = $(data).filter(\'meta[property="og:title"]\').attr("content");'."\n";
      echo '        var video = $(data).filter(\'meta[property="og:video"]\').attr("content");'."\n";
      ...
      echo '        $(\'#video_options\').append(\'<div class="input-group m-3"><div class="input-group-prepend"><span class="input-group-text text-monospace">MP4</span></div><div class="input-group-append"><a class="btn btn-primary" href="\'+ video + \'" role="button"><i class="fas fa-download fa-fw"></i> '.$download.'</a></div></div>\');'."\n";
      echo '      });'."\n";
      echo '    </script>'."\n";
      }
    }
